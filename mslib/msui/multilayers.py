    ...
    def _parse_itimes(self):
        ...
        if len(init_time_names) > 0:
            ...
            if len(self.allowed_init_times) == 0:
                logging.error("Cannot determine init time format of % for %s" % (self.header.text(0), self.text(0)))
    ...
    def _parse_vtimes(self):
        ...
        if len(valid_time_names) > 0:
            ...
            if len(self.allowed_valid_times) == 0:
                logging.error("Cannot determine valid time format of % for %s" % (self.header.text(0), self.text(0)))
                self.is_invalid = True
            else:
                if self.itime:
