...
class Plotting():
    def __init__(self):
        ...
        for flight, section, vertical, filename, init_time, time in \
            ...
            self.wps = self.load_from_ftml(filename)
            ...
            self.bm = mslib.msui.mpl_map.MapCanvas(ax=self.ax, **(self.params["basemap"]))
    ...
    def load_from_ftml(self, filename):
        ...
        wp_list = self.load_from_xml_data(datasource)
        return wp_list

    def load_from_xml_data(self, datasource):
        try:
