...
class Test_Mscolab:
    ...
    def test_upload_and_fetch_profile_image(self, qtbot):
        self._connect_to_mscolab(qtbot)
        self._login(qtbot, self.userdata[0], self.userdata[2])
        self.window.mscolab.profile_action.trigger()
        ...
        # Path where the image will be temporarily stored
        temp_image_path = os.path.join(os.getcwd(), 'new_profile_image.jpg')

        # Saving the imaeg on path
        with open(temp_image_path, 'wb') as f:
            f.write(img_byte_arr.getvalue())

        # Mocking the QFileDialog to select the image
        with mock.patch('PyQt5.QtWidgets.QFileDialog.getOpenFileName', return_value=(temp_image_path, 'Image (*.jpg)')):
